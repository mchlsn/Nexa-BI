...
export default function ChatbotWidget({ open, setOpen }) {
  // We keep messages, input, and loading in local state but let open/setOpen be managed via props
    ...
    <>
      ...
      {open && (
        <>
          ...
          <div
            className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 md:hidden transition-opacity duration-300 animate-fade-in"
            onClick={() => setOpen(false)}
          />

          <div
            id="chatbot-window"
            className="fixed z-50 flex flex-col overflow-hidden bg-[#1a1d27] border-l md:border border-[#2a2d3a] md:rounded-2xl shadow-2xl bottom-0 right-0 top-0 w-full sm:w-[400px] h-full md:bottom-24 md:right-5 md:top-auto md:w-96 md:h-[480px] animate-slide-left md:animate-slide-up"
            style={{
